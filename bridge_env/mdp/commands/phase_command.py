...
class PhaseCommand(CommandTerm):
    ...
    def __init__(self, cfg: PhaseCommandCfg, env: ManagerBasedEnv):
        ...
        self.base_command_cfg = self.cfg.base_command_cfg
        ...
        self.phase_command = torch.zeros(env.num_envs, self.num_phase_clock, device=env.device)
    ...
    def command(self) -> torch.Tensor:
        ...
        return torch.cat([self.base_command.command, self.phase_command], dim=1)
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        ...
        if self.cfg.randomize_start_phase:
            self.start_phase[env_ids] = torch.rand_like(env_ids)
    ...
    def _update_command(self):
        ...
        if self.cfg.stand_walk_switch:
            self.phase_command[:] *= ~self.is_standing_env
