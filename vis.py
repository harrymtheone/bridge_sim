...
def gen_info_panel(env: ParkourEnv):
    return Table.grid()

    global real_vx_avg, real_vy_avg, real_vz_avg, real_yaw_avg
    cmd_vx, cmd_vy, cmd_yaw, _, _ = env.command_manager.default_term.command[env.lookat_id].cpu().numpy()
    real_vx, real_vy, real_vz = env.robot.root_lin_vel_b[env.lookat_id].cpu().numpy()
    _, _, real_yaw = env.robot.root_ang_vel_b[env.lookat_id].cpu().numpy()
    ...
    # phase = env.command[env.lookat_id] if hasattr(env, 'phase') else 0.
    ...
    table11 = Table()
    table11.add_column(f'time: {env.episode_length_buf[env.lookat_id].item() * env.step_dt:.2f}')
...
class T1ActionsVisualizer(BaseVisualizer):
    ...
    def plot(self, env: ParkourEnv):
        actions = env.action_manager.default_action.cpu().numpy()
        self._plot({
